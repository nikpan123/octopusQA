...
export function belongsToCleanupBatch(run, batchId) {
  return Boolean(batchId) && run.cleanupBatchId === batchId && run.result === 'PASS'
    && Boolean(run.teacherId) && !['DELETED', 'ALREADY_ABSENT'].includes(run.cleanupStatus);
}
...
export default async function teardown() {
  ...
  if (selected.length) console.log(`Koniec testów — sprzątanie ${selected.length} nauczycieli z bieżącego uruchomienia.`);
