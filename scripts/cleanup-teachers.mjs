...
export async function cleanupTeacherBatch(page, selected, save) {
  ...
  for (const entry of selected) {
    ...
    if (previous && previous.run.email !== entry.run.email) throw new Error('Sprzeczne rejestry tego samego nauczyciela.');
  ...
  for (const {run} of unique.values()) {
    ...
    const tested = await api(page, '/api/Teacher/GetTeacherIsTested', 'GET', {teacherId:run.teacherId});
    if (tested.status !== 200 || tested.data !== true) throw new Error(`Nauczyciel ${run.teacherId}: brak flagi Testowy; nie wykonano DELETE.`);
  ...
  try {
    ...
    if (result.status < 200 || result.status >= 300) throw new Error(`Zbiorcze usuwanie zwróciło HTTP ${result.status}.`);
  } catch (error) {
  ...
  if (failures.length) throw new Error(`Nie potwierdzono usunięcia nauczycieli: ${failures.join(', ')}. ${deleteError ? 'Wynik żądania jest niepewny; serwer mógł nadal przetwarzać operację. ' : ''}Nie ponawiano DELETE.`, { cause: deleteError });
}
