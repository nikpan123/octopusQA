...
import {
  ...
  hasActiveSession,
  restoreSession,
  sessionHasMinimumLifetime,
} from "./auth.mjs";

const octopus = "https://octopus.gwodev.pl";
const gitlab = "https://gitlab.gwo.pl";
const secrets = {
  GITLAB_USERNAME: "gitlab-test-user",
  GITLAB_PASSWORD: "gitlab-test-password",
  OCTOPUS_USERNAME: "octopus-test-user",
  OCTOPUS_PASSWORD: "octopus-test-password",
};
...
function authSessionExpiringAt(exp) {
  ...
  return {
    storageState: {
      cookies: [],
      origins: [
        { origin: octopus, localStorage: [{ name: "token", value: JSON.stringify(token) }] },
      ],
    },
    ...
    try {
      ...
      await context.route("**/*", async (route) => {
        ...
        const redirect = (location) =>
          route.fulfill({
            ...
            body: `<script>location.href=${JSON.stringify(location)}</script>`,
          });
        const html = (body) => route.fulfill({ contentType: "text/html", body });
        if (req.method() === "POST") {
          submissions.push({
            origin: url.origin,
            data: Object.fromEntries(new URLSearchParams(req.postData())),
          });
          if (url.origin === gitlab) return redirect(`${octopus}/login`);
          signedIn = true;
          return redirect(`${octopus}/teacher/teacher-panel`);
        }
        if (url.origin === gitlab)
          return html(
            '<form method="post"><input id="user_login" name="login"><input id="user_password" name="password" type="password"><button>Sign in</button></form>',
          );
        if (url.pathname === "/login")
          return html(
            '<form method="post"><input id="login" name="login"><input id="password" name="password" type="password"><button>Zaloguj</button></form>',
          );
      ...
      if (withGitlab)
        assert.deepEqual(submissions[0], {
          origin: gitlab,
          data: { login: secrets.GITLAB_USERNAME, password: secrets.GITLAB_PASSWORD },
        });
      assert.deepEqual(submissions.at(-1), {
        origin: octopus,
        data: { login: secrets.OCTOPUS_USERNAME, password: secrets.OCTOPUS_PASSWORD },
      });
...
test("sessionStorage odtwarzany tylko dla Octopusa", async () => {
  ...
  try {
    ...
    await context.route("**/*", (route) =>
      route.fulfill({ contentType: "text/html", body: "<p>Test</p>" }),
    );
...
test("ekran logowania natychmiast oznacza zapisaną sesję jako nieaktualną", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      "Rozpoznanie /login nie powinno czekać na timeout panelu.",
    );
  } finally {
    await browser.close();
  }
