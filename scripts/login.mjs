...
try {
  ...
  await page.waitForURL((url) => url.origin === origin && /^\/(teacher|school)\//.test(url.pathname), {
    timeout: 600_000,
  });
  ...
  state.cookies = state.cookies.filter(
    (c) => c.domain.replace(/^\./, '') === 'octopus.gwodev.pl' || c.domain.replace(/^\./, '') === 'gwodev.pl',
  );
  ...
  await writeFile(path.join(authDir, 'session.json.tmp'), JSON.stringify({ origin, values: sessionStorage }), {
    mode: 0o600,
  });
