...
export function buildInventory(logs) {
  ...
  const add = (kind, id, file, result, parentId, name, cleanupStatus) => {
    ...
    item.references.push({ file, result: result ?? 'UNKNOWN', ...(parentId ? { parentId: String(parentId) } : {}), ...(name ? { name } : {}), ...(cleanupStatus ? { cleanupStatus } : {}) });
  ...
  for (const { file, data: d } of logs) {
    ...
    add('teacher', d.teacherId, file, d.result, d.schoolId, d.editedLastName ?? d.lastName ?? d.id, d.cleanupStatus);
  ...
  return [...records.values()].map(item => ({
    ...item,
    review: item.references.every(r => ['DELETED', 'ALREADY_ABSENT'].includes(r.cleanupStatus)) ? 'ABSENCE_CONFIRMED'
      : item.references.some(r => !['PASS', 'SHARED'].includes(r.result))
      ? 'KEEP_FOR_DIAGNOSIS' : 'VERIFY_IN_OCTOPUS',
  }));
}

async function main() {
  if (process.argv.length > 2) throw new Error('Podgląd nie przyjmuje parametrów i nie wykonuje usuwania.');
  const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
  const runs = path.join(root, 'runs');
  const files = await readdir(runs).catch(e => { if (e.code === 'ENOENT') return []; throw e; });
  const logs = [];
  const unreadable = [];
  for (const file of files.filter(f => /^REG_[\w-]+\.json$/.test(f)).sort()) {
    try { logs.push({ file, data: JSON.parse(await readFile(path.join(runs, file), 'utf8')) }); }
  ...
  console.table(records.map(r => ({ typ: r.kind, ID: r.id, rejestry: r.references.length, status: r.review })));
  ...
  if (files.length) {
    ...
    await writeFile(output, JSON.stringify({ generatedAt: new Date().toISOString(), mode: 'preview-only', unreadable, records }, null, 2));
