...
test('EDIT-03: zapis nazwiska jest trwały i widoczny w historii @teacher @edit', async ({ page, scenario: s, school }) => {
...
for (const kind of ['email', 'phone'] as const) {
  test(`TEA-04: niepoprawny ${kind === 'email' ? 'e-mail' : 'telefon'} blokuje zapis nauczyciela @teacher @validation`, async ({ page, scenario: s, school }) => {
    const form = await s.app.prepareTeacher(s.id, kind === 'email' ? 'invalid-email' : s.email, school.id, school.name);
    await test.step('Sprawdź błędne pole i komunikat przy próbie zapisu', async () => {
      const field = kind === 'email'
        ? form.locator('.new-teacher__personal-input').filter({ has: page.getByText('E-mail', { exact: true }) }).locator('input')
      ...
      await form.getByRole('button', { name: 'Zapisz', exact: true }).click();
      await expect(form.getByText(kind === 'email' ? 'Wprowadź poprawny adres E-Mail' : 'Wprowadź poprawny numer telefonu', { exact: true })).toBeVisible();
...
for (const criterion of ['Email', 'Nazwisko'] as const) {
  test(`FIND-05: wyszukiwanie nauczyciela — ${criterion} @teacher @search`, async ({ page, scenario: s, school }) => {
    const teacherId = await s.createTeacher(school.id, school.name);
    ...
    await expect(results.getByRole('row').filter({ has: page.getByRole('gridcell') })).toHaveCount(1);
    const row = results.getByRole('row').filter({ has: page.getByRole('gridcell', { name: teacherId, exact: true }) });
...
test('REL-02: druga szkoła zachowuje pierwszą relację i pokazuje nauczyciela w obu szkołach @teacher @relation', async ({ page, scenario: s, school }) => {
  ...
  await test.step('Potwierdź trwałość dokładnie dwóch powiązań', async () => {
    await s.app.openPanel('teacher', teacherId);
    const schools = page.getByRole('treegrid').filter({ has: page.getByRole('columnheader', { name: 'Nazwa szkoły', exact: true }) });
  ...
  await test.step('Znajdź tego samego nauczyciela w obu kartotekach szkół', async () => {
    for (const id of [school.id, secondId]) {
      ...
      const row = teachers.getByRole('row').filter({ has: page.getByRole('gridcell', { name: teacherId, exact: true }) });
      await expect(row).toHaveCount(1);
