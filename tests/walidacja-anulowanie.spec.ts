...
test.describe('Walidacja wymaganych danych @validation', () => {
  for (const missing of ['imię', 'nazwisko', 'szkoła', 'kontakt'] as const) {
    test(`TEA-02: brak pola ${missing} blokuje zapis nauczyciela @validation`, async ({ page, scenario: s }) => {
      const schoolId = missing === 'szkoła' ? undefined : await s.createSchool();
      const form = await s.app.prepareTeacher(s.id, missing === 'kontakt' ? '' : s.email, schoolId, s.schoolName);
...
test.describe('Anulowanie formularzy @cancel', () => {
  test('TEA-03: anulowanie kompletnego formularza nie tworzy nauczyciela @cancel', async ({ scenario: s }) => {
    ...
    await expect(s.app.page.getByRole('button', { name: 'Nauczyciele: 0', exact: true })).toBeVisible();
  });

  test('SCH-02: anulowanie kompletnego formularza nie tworzy szkoły @cancel', async ({ scenario: s }) => {
  ...
  test('EDIT-02: anulowanie edycji zachowuje dane i historię @cancel', async ({ page, scenario: s }) => {
    const schoolId = await s.createSchool();
    ...
    await expect(form).toHaveCount(0);
    await s.app.openPanel('teacher', teacherId);
    for (const [field, value] of Object.entries(before)) await expect(s.app.detail(field)).toHaveValue(value);
...
test.describe('Wyszukiwanie bez wyników @search', () => {
  for (const kind of ['teacher', 'school'] as const) {
    test(`FIND-04: brak wyników usuwa poprzednią listę — ${kind} @search`, async ({ page, scenario: s }) => {
      const schoolId = await s.createSchool();
      if (kind === 'teacher') {
        const teacherId = await s.createTeacher(schoolId);
