  ...
  try {
    ...
    await test.step("AUDIT-01: sprawdź wpis edycji i dodanie szkoły", async () => {
      ...
      const edit = history
        .getByRole("row")
        .filter({
          has: page.getByRole("gridcell", { name: "Imię", exact: true }),
        })
        .filter({
          has: page.getByRole("gridcell", { name: "Jan", exact: true }),
        })
        .filter({
    ...
    await test.step("REL-01: sprawdź tego samego nauczyciela od strony szkoły", async () => {
      ...
      await expect(
        teachers.getByRole("button", { name: "Nauczyciele: 1", exact: true }),
      ).toBeVisible();
